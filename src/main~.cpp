...
int song() {
    ...
    vector<double> Y;
    for (int i = 0; i < N; ++i) {
        X.emplace_back(xa + i * dx);
        Y.emplace_back(ya + i * dy);
    }
    vector<vector<double>> mesh = { X, Y };
    LegendrePoly P(2, 3, mesh);
    P.getCoeff();

    int coeffSize = P.numBasis;
    // cell number
    int N_x = mesh[0].size() - 1;
    int N_y = mesh[1].size() - 1;

    vector<vector<VectorXd>> coeff(N_x + 1, vector<VectorXd>(N_y + 1, VectorXd(coeffSize)));
    for (int i = 1; i < N_x + 1; ++i) {
        for (int j = 1; j < N_y + 1; ++j) {
            coeff[i][j] = Map<VectorXd>(P.coeff[i - 1][j - 1].data(), coeffSize);
        }
    }
    // periodic boundary condition
    for (int j = 0;j < N_y + 1; ++j) {
    ...
    for (int i = 0; i < coeffSize; ++i) {
        for (int j = 0; j < coeffSize; ++j) {
            ...
            B(i, j) += -(n[0] > m[0]) * ((n[0] - m[0]) % 2 == 1) * 2 * (m[1] == n[1]);
            B(i, j) += -(n[1] > m[1]) * ((n[1] - m[1]) % 2 == 1) * 2 * (m[0] == n[0]);

            // 使用 std::pow 进行幂运算
            D(i, j) = std::pow(-1, m[0] + n[0] + 1) * (m[1] == n[1]);
            E(i, j) = std::pow(-1, m[1] + n[1]) * (m[0] == n[0]);
        }
    }



    double t = CFL * pow(sqrt(dx * dy), (k + 1) / 3.0); // 确保除法是浮点数
    vector<double> T;
    T.emplace_back(0);
